...
const all = exports.all = () => {
    return _redis.redis.get(CACHE_NAME).then(function (result) {
        if (result && result !== '') {
            ...
            throw _boom2.default.serverUnavailable('all /Occupancy/CarPark endpoints are temporarily unavailable');
        }
    });
};
    ...
    var _ref = _asyncToGenerator(function* (carPark) {
        return _redis.redis.get(CACHE_NAME + '_' + carPark).then(function (result) {
            if (result && result !== '') {
                ...
                throw _boom2.default.serverUnavailable('Data from carpark [' + carPark + '] is temporarily unavailable');
            }
        });
    });
